...
import tink.io.hl.HlTlsSource;
import tink.io.hl.HlTlsSink;
...
class HlTlsConnection implements Connection {
  ...
  public function new(name:String, session:HlTlsSession, ?local:Endpoint, ?peer:Endpoint) {
    ...
    source = HlTlsSource.wrap('Incoming stream of $name', session);
    sink = HlTlsSink.wrap('Outcoming stream of $name', session);
  }
}
#end
