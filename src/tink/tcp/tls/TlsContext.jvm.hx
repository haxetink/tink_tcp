...
private class TlsContextData {
  ...
  public static function fromServer(options:TlsServerOptions):TlsContextData {
    ...
    return new TlsContextData(ctx, false, TlsAuth.serverMode(options), null, options.alpn);
  }

  public function newEngine(?host:String, ?port:Int):SSLEngine {
    if (isClient) {
...
abstract TlsContext(TlsContextData) from TlsContextData {
  ...
  public inline function newEngine(?host:String, ?port:Int):SSLEngine
    return this.newEngine(host, port);
}
