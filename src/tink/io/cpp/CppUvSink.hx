...
@:allow(tink.io.cpp)
class CppUvSink extends SinkBase<Error, Noise> {
  final target:CppUvStream;

  function new(target:CppUvStream) {
    this.target = target;
  }
  ...
  static public inline function wrap(name:String, target:CppUvStream):RealSink
    return new CppUvSink(target);
}
#end
